  ...
  return (
    <header className={styles.header}>
      <Link to="/" className={styles.logoContainer}>
        ...
        <span className={styles.logoBottom}>SINCE 1833</span>
      </Link>
      
      <div className={`${styles.menuContainer} ${isMobileMenuOpen ? styles.mobileOpen : ''}`}>
        <nav className={styles.navLinks}>
          <NavLink to="/our-story" onClick={toggleMenu} className={getNavClass('/our-story')}>Our Story</NavLink>
          <NavLink to="/inside-the-source" onClick={toggleMenu} className={getNavClass('/inside-the-source')}>Inside the Source</NavLink>
          <div className={styles.dropdownWrap}>
            <NavLink to="/shop" onClick={toggleMenu} className={getNavClass('/shop')}>
              Shop Water
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
            </NavLink>
          </div>
          <NavLink to="/wct" onClick={toggleMenu} className={getNavClass('/wct')}>WCT</NavLink>
          <NavLink to="/support" onClick={toggleMenu} className={getNavClass('/support')}>Support</NavLink>
        </nav>

        <div className={styles.actions}>
          <Link to="/login" className={styles.iconBtn} aria-label="User Account">
